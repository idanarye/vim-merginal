...
"
"FUNCTION: KeyMap.BindAll() {{{1
function! s:KeyMap.BindFor(scope)
    for i in s:KeyMap.All()
        call i.bind()
    endfor
endfunction
...
function! s:KeyMap.bind()
    ...
    exec 'nnoremap <buffer> <silent> '. self.key . premap . ':call merginal#invokeKeyMap("'. keymapInvokeString .'")<cr>'
endfunction
...
function! s:KeyMap.invoke(...)
    ...
    if(has_key(self, 'args'))
      call call(Callback,self.args)
    else
      call Callback()
    endif
endfunction
...
function! s:KeyMap.Invoke(key)
    ...
    " if !g:Merginal.ExistsForBuf()
    "     return {}
    " endif
    "find the appropriate key frm the keymap and hen invoke it
    let l:merginalBufferNumber=bufnr('%')
    let l:merginalBufferName=bufname(l:merginalBufferNumber)

    let km = s:KeyMap.FindFor(a:key, l:merginalBufferName)
    if !empty(km)
        return km.invoke()
    endif
